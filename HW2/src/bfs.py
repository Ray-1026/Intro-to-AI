...
def bfs(start, to):
    ...
    """
    Implement BFS.
    1. Initialize an empty dictionary called 'parent', a list with start ID called 'queue', a
       set to mark the visited node called 'visited', and 'num_visited' and 'isfind' to 0.
    2. Pop the element from the front of the list 'queue', add it to the set 'visited', and add
       1 to the 'num_visited'.
    3. Explore the neighbors of the popped node. For each unvisited neighbors, add them to the
       'visited' and to the 'queue', and record the popped node and the distance in the dictionary
       'parent'.
    ...
    2. If the variable 'isfind' is 1, which means BFS finds a path from 'start' to 'end' successfully,
